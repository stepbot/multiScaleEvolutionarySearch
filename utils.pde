...
void plotProblem(int problem){
  for(int ii = 0; ii < width; ii++){
    for(int jj = 0; jj < height; jj++){
      x = map(ii,0,width,minRange,maxRange);
      y = map(jj,0,height,maxRange,minRange);
      value = problemValue(problem,x,y);
...
void createAgents(int problem){
  for(int ii = 0; ii < agentCount; ii++){
      ...
      value = problemValue(problem,x,y); 
      agents[ii] = new agent(x,y,int(map(x,minRange,maxRange,0,width)),int(map(y,maxRange,minRange,0,height)),value,agentColor);
    }
}
...
void plotSearchPoint(){
  set(int(map(searchX,minRange,maxRange,0,width)),int(map(searchY,maxRange,minRange,0,height)),searchPointColor);
}